<x-app-layout>
    <x-slot name="page_title">{{ __('EM | Edit Event') }}</x-slot>
    <x-slot name="header">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('events.update', $event) }}"
                      ...
                      x-data="{
                        country: @js(old('country', $event->country->id)),
                        cityId: @js(old('city', $event->city->id)),
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <select name="country_id"
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" @selected(old('country', $event->country->id) === $country->id)>{{ $country->name }}</option>
                            @endforeach
                        </select>
                        @error('country_id')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <textarea name="description"
                                  ...
                                  cols="30"
                                  rows="10"
                                  class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">{{ old('description', $event->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                                ...
                                <li class="dark:border-gray-600">
                                    <div class="flex items-center ps-3">
                                        <input id="tag-{{ $tag->id }}"
                                               ...
                                               type="checkbox"
                                               value="{{ $tag->id }}"
                                               @checked(collect(old('tags', $event->tags))->pluck('id')->contains($tag->id))
